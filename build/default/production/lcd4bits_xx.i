...
void lcd_puts(const char *string, char *port) {
    char cmd;
    while (*string != 0)
        lcd_write(1, *string++, &cmd);
}


void lcd_putch(char character, char *port) {
    char cmd;
    lcd_write(1, character, &cmd);
}
...
void lcd_init(char *port)
{
    ...
    RD1 = 0;
    DelayMs(15);
    lcd_write(0, 0x28, &cmd);
    lcd_write(0, 0x0C, &cmd);
    lcd_clear(&cmd);
    lcd_write(0, 0x06, &cmd);
}
