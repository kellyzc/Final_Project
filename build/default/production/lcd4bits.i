...
void lcd_goto(unsigned char position, char *port) {
    lcd_write(0x00, 0x80 + position, port);
...
void lcd_init(char *port)
{
    ...
    *port = 0;
    DelayMs(15);
    lcd_write(0x00, 0x28, port);
    lcd_write(0x00, 0x0C, port);
    lcd_clear(port);
    lcd_write(0x00, 0x06, port);
}

void lcd_set_custom_char(const char *data, char cg_address, char *port) {
    char row;
    for(row = 0; row<8; row++) {
        lcd_write(0x00, 0x40|(cg_address<<3)|row, port);
        lcd_write(0x10, *data, port);
        *data++;
    }
